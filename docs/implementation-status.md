...
| Compatibility and build | Migrations 014–017 are additive; persisted `quiz`/checkpoint contracts, routes, storage keys, authentication, realtime event names, and versioned records remain compatible; optional `JoinRequest.avatarId`, purpose-aware practice/report DTOs, and participant avatar fields preserve older consumers, while existing state v4 snapshots receive a deterministic valid fallback without a game-state migration; ready Report V2 and V3 can create recovery follow-ups; a CI/local Docker-context guard checks that image builds include every transitive workspace manifest and source tree before install/build, and the corrected web image builds locally | Green clean-host PR/main CI plus production backup/migration/forward-repair rehearsal                                 |

Within repository scope, no planned P0 beta capability remains unimplemented. The remaining
beta-exit cells above require observed usability, manual assistive-technology, independent review,
hosted performance, or operational evidence; passing local automation does not close them.

The first customer-facing P1 authoring slice is deliberately bounded. Question reuse copies the
...
merged baseline. It intentionally keeps single-VM staging, privacy/legal approval, design-partner
observation, manual VoiceOver/NVDA and physical-device coverage, target-host load/soak, encrypted
off-host backup and clean replacement-VM restore, real-provider source-corpus evaluation, and
provider-originated Stripe rehearsal pending. Local automation does not satisfy those gates.
